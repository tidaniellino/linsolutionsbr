...
app.post('/api/schedule', async (req, res) => {
  try {
    ...
    // 3. Gerar convite de calendário (.ics)
    const eventDetails = {
    ...
    const adminEmailHtml = `
      <div style="font-family: Arial, sans-serif; background-color: #0C0101; color: #FFF5F5; padding: 25px; border-radius: 8px;">
        ...
        <p style="font-size: 0.85rem; color: rgba(255,245,245,0.7);">
          O horário foi inserido na agenda da conta <strong>${ADMIN_EMAIL}</strong> e o e-mail de confirmação com anexo .ics já foi encaminhado para o cliente.
        </p>
      </div>
    `;

    // --------------------------------------------------------------------------
    // 5. DISPARO DE E-MAIL 2: Para o Cliente (Confirmação)
    // --------------------------------------------------------------------------
